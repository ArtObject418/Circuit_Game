...
func _ready():
	...
	$PanelContainer.set_title("Уровень 4")
	...
	$Teoretic.set_description("Кадая точка в цепи имеет свой электрический потенициал, который измеряется в Вольтах(В)\n\nНам важны лишь потенциалы на выходах цепи")
...
func check_completed():
	var f1 = global_pivot($BLOCKS/TextureButton) == $POSITIONS/Position2D5.global_position
	var f2 = global_pivot($BLOCKS/TextureButton2) in [$POSITIONS/Position2D2.global_position, $POSITIONS/Position2D3.global_position]
	var f3 = global_pivot($BLOCKS/TextureButton3) == $POSITIONS/Position2D4.global_position
	var f4 = global_pivot($BLOCKS/TextureButton4) == $POSITIONS/Position2D11.global_position
	var f5 = global_pivot($BLOCKS/TextureButton5) in [$POSITIONS/Position2D2.global_position, $POSITIONS/Position2D3.global_position]
	print(f1,f2,f3,f4,f5)
	if f1 and f2 and f3 and f4 and f5:
		SaveManager.save_progress(5)
