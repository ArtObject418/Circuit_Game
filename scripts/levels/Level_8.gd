...
func _ready():
	...
	$PanelContainer.set_title("Уровень 8")
	...
	$Teoretic.set_description("Для измерения силы тока на участке используется амперметр")
...
func check_completed():
	var f1 = global_pivot($BLOCKS/TextureButton) == $POSITIONS/Position2D21.global_position
	var f2 = global_pivot($BLOCKS/TextureButton2) == $POSITIONS/Position2D19.global_position
	var f3 = global_pivot($BLOCKS/TextureButton3) in [$POSITIONS/Position2D2.global_position, $POSITIONS/Position2D4.global_position, $POSITIONS/Position2D11.global_position]
	var f4 = global_pivot($BLOCKS/TextureButton4) in [$POSITIONS/Position2D2.global_position, $POSITIONS/Position2D4.global_position, $POSITIONS/Position2D11.global_position]
	var f5 = global_pivot($BLOCKS/TextureButton5) in [$POSITIONS/Position2D2.global_position, $POSITIONS/Position2D4.global_position, $POSITIONS/Position2D11.global_position]
	print(f1,f2,f3,f4,f5)
	if f1 and f2 and f3 and f4 and f5:
		$Ammetr.text = '1A'
		SaveManager.save_progress(9)
